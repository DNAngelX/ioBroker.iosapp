...
class Iosapp extends utils.Adapter {
    constructor(options) {
        super({
            ...options,
            name: 'iosapp',
        ...
        try {
            ...
            for (const id in objects) {
                if (objects.hasOwnProperty(id)) {
                    const personMatch = id.match(/^iosapp\.\d+\.person\.([^\.]+)/);
                    if (personMatch) {
        ...
        try {
            ...
            for (const id in objects) {
                if (objects.hasOwnProperty(id) && !id.includes('.messages')) {  // Ignore devices named "messages"
                    const deviceMatch = id.match(/^iosapp\.\d+\.person\.[^\.]+\.([^\.]+)/);
                    if (deviceMatch) {
    ...
    async handleGetZones(socket) {
        ...
        try {
            ...
            for (const id in objects) {
                if (objects.hasOwnProperty(id)) {
                    const zoneMatch = id.match(/^iosapp\.\d+\.zones\.([^\.]+)/);
                    if (zoneMatch) {
    ...
    async handleAPNMessage(id) {
        ...
        if (device === 'messages') {
            ...
            clientIds = Object.values(deviceStates).map(state => state ? state.val : null).filter(val => val);
        } else if (person === 'payload') {
            basePath = `${this.namespace}.messages`;
            const allDeviceStates = await this.getStatesAsync(`${this.namespace}.person.*.*.ws_device_id`);
            clientIds = Object.values(allDeviceStates).map(state => state ? state.val : null).filter(val => val);
        } else {
            basePath = `${this.namespace}.person.${person}.${device}.messages`;
    ...
    async setConnectionState(wsDeviceId, connected) {
        try {
            ...
            for (const personId in persons) {
                const personParts = personId.split('.');
                const person = personParts[3];
                
                const devices = await this.getForeignObjectsAsync(`${this.namespace}.person.${person}.*`, 'channel');
                for (const deviceId in devices) {
                    const deviceParts = deviceId.split('.');
                    const device = deviceParts[4];
                    
                    const state = await this.getStateAsync(`${this.namespace}.person.${person}.${device}.ws_device_id`);
                    
                    if (state && state.val === wsDeviceId) {
                        const statePath = `${this.namespace}.person.${person}.${device}.connection`;
                        ...
                        return; // Exit once the correct ws_device_id is found
                    }
                }
            }
...
if (module.parent) {
    module.exports = (options) => new Iosapp(options);
} else {
    new Iosapp();
}
